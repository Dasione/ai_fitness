...
def process_video(video_path, point1, point2, output_csv, output_p, alingn_csv, std_csv, std, model_path_data):
    ...
    if len(adjusted_peaks) > 0:
        ...
        segment_ranges.append((adjusted_peaks[-1], len(frames) - 1))
        # 检查最后一段动作的角度数据
        last_segment_start = adjusted_peaks[-1]
        last_segment_angles = angles_group1[last_segment_start:]
        zero_count = np.sum(last_segment_angles == 0)
        
        if zero_count < 8:  # 如果0值少于8个，保留最后一段
            segment_ranges.append((adjusted_peaks[-1], len(frames) - 1))
            last_peak_frame = len(frames) - 1
        else:  # 如果0值达到或超过8个，舍弃最后一段
            last_peak_frame = adjusted_peaks[-1]
    else:
        segment_ranges = []
        last_peak_frame = len(frames) - 1
